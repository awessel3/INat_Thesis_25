...
#Starting Analysis w/ just COLLIN ------

COLLIN_wo_annotations <- read.csv("Data/COLLIN_wo_annotations.csv")

COLLIN_no_flowering <- read.csv("Data/COLLIN_no_flowering.csv")

COLLIN_12_13 <- read.csv("Data/COLLIN_12_13.csv")
COLLIN_wo12 <- read.csv("Data/COLLIN_wo12.csv")

#Removing only non-flowering observation. 
## What is a more efficient way of doing this? 
COLLIN <- COLLIN_wo12 %>% filter(id != 222132118)

# Adding Year, Month, and doy to each observation 
COLLIN <- COLLIN %>%
  replace(is.na(.),"0") %>%  # not working - why?  because it's a character field and was trying to put number
...
head(COLLIN,2)
str(COLLIN)
table(COLLIN$winter.month)

COLLIN <- COLLIN %>%  mutate(year.factor=as.factor(year), 
                             winter.year=as.factor(winter.year),
                             field.flowering.phenology = 1)

COLLIN <- droplevels(filter(COLLIN, year > 2018, year<2024))
